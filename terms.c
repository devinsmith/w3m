...
setupscreen(void)
{
	...
	if (COLS + 1 > max_COLS) {
		...
		for (i = 0; i < max_LINES; i++) {
			...
			memset(ScreenElem[i].lineimage, 0,
			    max_COLS * sizeof(char *));
#else
			ScreenElem[i].lineimage = New_N(char, max_COLS);
#endif
...
#ifndef USE_M17N
	/*
	 * Eliminate unprintables according to * iso-8859-*. Particularly
	 * 0x96 messes up T.Dickey's * (xfree-)xterm
	 */
	if (IS_INTSPACE(c))
		c = ' ';
#endif
...
color_seq(int colmode)
{
	...
	snprintf(seqbuf, sizeof(seqbuf), "\033[%dm",
	    ((colmode >> 8) & 7) + 30);
	return seqbuf;
}
		...
		if (*dirty & L_DIRTY) {
			*dirty &= ~L_DIRTY;
			...
			for (; col < COLS; col++) {
				 ...
				 * character is put on COLS-th column. this
				 * behavior is different from one of vt100,
				 * but such terminal emulators are used as
				 * vt100-compatible emulators. This behaviour
				 * causes scroll when a character is drawn on
				 * (COLS-1,LINES-1) point.  To avoid the
				 * scroll, I prohibit to draw character on
				 * (COLS-1,LINES-1).
				 */
...
#ifdef __EMX__
	/*
	 * On XFree86/OS2, some scrambled characters will appear when
	 * asserting IEXTEN flag.
	 */
