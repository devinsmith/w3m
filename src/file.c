...
#define cur_form_id ((form_sp >= 0)? form_stack[form_sp] : -1)
	...
	static off_t current_content_length;
...
readHeader(URLFile * uf, Buffer * newBuf, int thru, ParsedURL * pu)
{
	...
	while ((tmp = StrmyUFgets(uf))->length) {
		...
		if (!strncasecmp(lineBuf2->ptr, "content-transfer-encoding:", 26)) {
			...
			Str name = Strnew(), value = Strnew(), domain = NULL,
			 path = NULL, comment = NULL, commentURL = NULL,
			 port = NULL, tmp2;
			...
			while (*p == ';') {
				...
				if (matchattr(p, "expires", 7, &tmp2)) {
					...
					/*
					 * XXX Is there any problem with
					 * max-age=0? (RFC 2109 ss. 4.2.1,
					 * 4.2.2
					 */
 ...
 * = "qop" "=" qop-value cnonce		   = "cnonce" "=" cnonce-value
 * cnonce-value	 = nonce-value nonce-count	  = "nc" "=" nc-value
 * nc-value		 = 8LHEX response		 = "response" "="
 * request-digest request-digest = <"> 32LHEX <"> LHEX			 =
	       ...
	       HRequest * hr, FormList * request)
{
	...
	Str cnonce		/* = qstr_unquote(get_auth_param(ha->param,
	    "cnonce")) */ ;
	...
	if (qop_i >= QOP_AUTH) {
		...
		/*
		 * compatibility with RFC 2069 request_digest = KD(H(A1),
		 * unq(nonce), H(A2))
		 */
		tmp = Strnew_m_charp(a1buf->ptr, ":",
	...
	if (qop_i >= QOP_AUTH) {
		...
		/* XXX how to count? */
		/*
		 * Since nonce is unique up to each *-Authenticate and w3m
		 * does not re-use *-Authenticate: headers, nonce-count
		 * should be always "00000001".
		 */
 ...
 *  "opaque" "=" quoted-string stale			 = "stale" "=" (
 * "true" | "false" ) algorithm		 = "algorithm" "=" ( "MD5" |
 * "MD5-sess" | token ) qop-options	   = "qop" "=" <"> 1#qop-value <">
...
load_doc:
	...
	/*
	 * XXX: can we use guess_type to give the type to loadHTMLstream to
	 * support default utf8 encoding for XHTML here?
	 */
	f.guess_type = t;
	  ...
	  int force, int width)
{
	...
	char *hidden_anchor = NULL, *hidden_img = NULL, *hidden_bold = NULL, *hidden_under = NULL,
	*hidden_italic = NULL, *hidden_strike = NULL, *hidden_ins = NULL,
	*hidden = NULL;
...
process_input(struct parsed_tag * tag)
{
	...
	if (!q) {
		...
		case FORM_INPUT_RESET:
			 ...
			 * TYPE=CHECKBOX> tag, then the value "on" is used as
			 * a default value. It is not a part of HTML4.0
			 * specification, but an imitation of Netscape
			 * behaviour.
			 */
...
proc_normal:
		if (obuf->table_level >= 0) {
			/*
			 * within table: in <table>..</table>, all input
			 * tokens are fed to the table renderer, and then the
			 * renderer makes HTML output.
			 */
		...
		if (is_tag) {
			...
			if (HTMLtagproc1(tag, h_env) == 0) {
				/*
				 * preserve the tag for second-stage
				 * processing
				 */
				if (parsedtag_need_reconstruct(tag))
...
static void
addnewline2(Buffer * buf, char *line, Lineprop * prop, int pos,
	    int nlines)
{
	Line *l;
	l = New(Line);
...
static void
addnewline(Buffer * buf, char *line, Lineprop * prop, int pos,
...
char *
convert_size(off_t size, int usefloat)
{
	float csize;
	int sizepos = 0;
...
char *
convert_size2(off_t size1, off_t size2, int usefloat)
{
	char **sizes = _size_unit;
...
void
showProgress(off_t *linelen, off_t *trbyte)
{
...
getNextPage(Buffer * buf, int plen)
{
	...
	off_t linelen = 0, trbyte = buf->trbyte;
	Str lineBuf2;
