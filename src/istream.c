...
static void file_close(struct w3s_file_handle * handle);
static int file_read(struct w3s_file_handle * handle, char *buf, int len);
...
newFileStream(FILE * f, void (*closep) ())
{
	...
	stream->file.handle = New(struct w3s_file_handle);
...
static void
file_close(struct w3s_file_handle * handle)
{
	handle->close(handle->f);
}

static int
file_read(struct w3s_file_handle * handle, char *buf, int len)
{
