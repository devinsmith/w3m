...
void
 isrch(int (*func) (Buffer *, char *), char *prompt) {
	...
	Buffer sbuf;
	SAVE_BUFPOSITION(&sbuf);
	dispincsrch(0, NULL, NULL);	/* initialize incremental search
					 * state */
...
loadLink(char *url, char *target, char *referer, FormList * request)
{
	...
	if (target == NULL ||	/* no target specified (that means this page
				 * is not a frame page) */
...
do_submit:
		...
		if (fi->parent->method == FORM_METHOD_GET) {
			...
			if (buf && !(buf->bufferprop & BP_REDIRECTED)) {	/* buf must be
										 * Currentbuf */
				/*
				 * BP_REDIRECTED means that the buffer is
				 * obtained through Location: header. In this
				 * case, buf->form_submit must not be set
				 * because the page is not loaded by POST
				 * method but GET method.
				 */
...
checkBackBuffer(Buffer * buf)
{
	...
	if (fbuf) {
		...
		/*
		 * when no frames stacked and next is frame source, try
		 * next's nextBuffer
		 */
...
void
addDownloadList(pid_t pid, char *url, char *save, char *lock, off_t size)
{
	DownloadList *d;
