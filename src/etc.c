	...
	for (p = *instr; *p; p++) {
		...
		case R_ST_TAG:
			...
			if (*status == R_ST_TAG0 && !REALLY_THE_BEGINNING_OF_A_TAG(p)) {
				/*
				 * it seems that this '<' is not a beginning
				 * of a tag
				 */
				/*
				 * Strcat_charp(buf, "&lt;");
...
openSecretFile(char *fname)
{
	...
	/*
	 * check permissions, if group or others readable or writable, refuse
	 * it, because it's insecure.
	 * 
	 * XXX: disable_secret_security_check will introduce some security
	 * issues, but on some platform such as Windows it's not possible (or
	 * feasible) to disable group|other readable and writable. [w3m-dev
	 * 03368][w3m-dev 03369][w3m-dev 03370]
	 */
...
void (*
      mySignal(int signal_number, void (*action) (int))) (int)
{
#ifdef	SA_RESTART
